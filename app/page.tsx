    ...
    <main className="relative min-h-screen overflow-x-hidden bg-[#F8FAFC] px-4 text-slate-950 sm:px-6 lg:px-8">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col">
        ...
        <section className="flex flex-1 flex-col items-center justify-start pb-7 pt-7 text-center sm:justify-center sm:pb-8 sm:pt-0">
          <h1 className="max-w-[340px] animate-[fadeInUp_0.7s_ease-out_0.1s_both] text-4xl font-black leading-[1.08] tracking-[-0.045em] text-emerald-600 sm:max-w-full sm:text-5xl md:max-w-3xl md:whitespace-nowrap md:text-6xl lg:text-6xl">
            Kelola Warung Lebih Rapi
          </h1>

          <p className="mt-3 max-w-[320px] animate-[fadeInUp_0.7s_ease-out_0.18s_both] text-sm leading-6 text-slate-600 sm:max-w-xl sm:text-base md:max-w-3xl md:whitespace-nowrap">
          ...
          <div className="mt-8 grid w-full max-w-5xl animate-[fadeInUp_0.7s_ease-out_0.32s_both] grid-cols-1 gap-3 sm:mt-10 sm:grid-cols-2 lg:grid-cols-4">
            {features.map((feature) => {
              ...
              return (
                <Card
                  key={feature.title}
                  className="group rounded-[24px] border-slate-200/80 bg-white/85 text-left shadow-[0_14px_45px_rgba(15,23,42,0.055)] backdrop-blur transition-all duration-300 hover:-translate-y-1 hover:border-emerald-200 hover:shadow-[0_22px_64px_rgba(16,185,129,0.13)] sm:min-h-[160px] sm:rounded-[26px]"
                >
                  <CardContent className="flex h-full flex-col p-4 sm:p-4">
                    <div className="flex h-9 w-9 items-center justify-center rounded-2xl bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100 transition-colors group-hover:bg-emerald-600 group-hover:text-white">
                      <Icon className="h-4.5 w-4.5" />
                    </div>
                    <h2 className="mt-4 text-base font-extrabold tracking-tight text-slate-950">
                      {feature.title}
                    </h2>
